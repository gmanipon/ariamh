...
ID_TMPL = "S1-IFG_STITCHED_TN{:03d}_{}-{}_s{}-{}"
...
def group_frames_by_track_date(frames):
    ...
    for h in frames: 
        ...
        hits[h['_id']] = prod_url
        match = IFG_RE.search(h['_id'])
        if not match:
            raise RuntimeError("Failed to recognize IFG ID %s." % h['_id'])
        ...
        dt_pair_key = "%s_%s" % (start_day_dt.strftime('%Y%m%d'), end_day_dt.strftime('%Y%m%d'))
        bisect.insort(grouped.setdefault(fields['metadata']['trackNumber'], {}) \
                             .setdefault(dt_pair_key, {}) \
                             .setdefault(fields['metadata']['swath'], []),
                      ("%s_%s" % (ifg_start_dt.strftime('%Y%m%dT%H%M%S'),
...
def get_stitch_cfgs(context_file):
    ...
    direction = context.get('direction', 'across')
    ...
    ## build stitcher inputs
    #valid_ts_out_file = "valid_ts_out.json"
    #inps = {
    #    "swaths": [1,2,3],
    #    "latitudeIndexMin": int(bbox[0] * 10),
    #    "latitudeIndexMax": int(bbox[1] * 10),
    #    "output_file": valid_ts_out_file,
    #    "meta_file": valid_meta_ts_out_file,
    #}
    #valid_ts_in_file = "valid_ts_in.json"
    #with open(valid_ts_in_file, 'w') as f:
    #    json.dump(inps, f, indent=2)
    # 
    ## validate
    #validate_cmd = [
    #    "python3", "{}/stitcher_utils.py".format(BASE_PATH),
    #    "-a", "validate_ts_met", "-i", valid_ts_in_file
    #]
    #validate_cmd_line = " ".join(validate_cmd)
    #logger.info("Calling stitcher_utils.py validate_ts_met: {}".format(validate_cmd_line))
    #check_call(validate_cmd_line, shell=True)
    ...
    for track in sorted(grouped['grouped']):
        for dt_pair in sorted(grouped['grouped'][track]):
            ...
            ifg_sts = []
            ifg_ets = []
            for swath in sorted(grouped['grouped'][track][dt_pair]):
                swathnums.append(swath)
                fnames_swath = []
                for tr, id in grouped['grouped'][track][dt_pair][swath]:
                    fnames_swath.append(os.path.join(id, 'merged', 'filt_topophase.unw.geo'))
            ...
            ifg_ids.append(ID_TMPL.format(
                int(track), ifg_sts[0], ifg_ets[-1], 
                ''.join(map(str, swathnums)), ifg_hash[0:4]))
