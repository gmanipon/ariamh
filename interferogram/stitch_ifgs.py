...
def create_dataset_json(id, version, ds_files, ds_json_file):
    ...
    logger.info("union polygon: {}.".format(json.dumps(location, indent=2)))
...
def create_met_json(id, version, env, met_files, met_json_file):
    ...
    set_params = ('master_scenes', 'esd_threshold', 'frameID', 'swath', 'parallelBaseline',
                  ...
                  'orbitNumber', 'perpendicularBaseline', 'orbitRepeat', 'sensingStop',
                  'polarization', 'sensor', 'lookDirection', 'platform', 'startingRange',
                  'sensingStart', 'beamMode', 'direction', 'prf' )
...
def main():
    ...
    # get args
    project = ctx['project']
    direction = ctx.get('direction', 'across')
    ...
    envelope = create_dataset_json(id, version, dsets, ds_json_file)

    # create met json
    met_json_file = os.path.join(dataset_dir, "{}.met.json".format(id))
    create_met_json(id, version, envelope, mets, met_json_file)
